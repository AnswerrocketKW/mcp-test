...
install_mcp_servers() {
    ...
    while IFS='|' read -r copilot_id copilot_name; do
        if [[ -n "$copilot_id" ]]; then
            ...
            if uv run mcp install main.py -n "$server_name" -v "AR_URL=$ar_url" -v "AR_TOKEN=$ar_token" -v "COPILOT_ID=$copilot_id" --with "git+ssh://git@github.com/answerrocket/answerrocket-python-client.git@get-copilots-for-mcp"; then
